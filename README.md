...
**Запуск тестов в единственном экземпляре браузера. Написание третьего теста**

1. Добавить в **ApplicationManager** переменную **ThreadLocal<ApplicationManager>**.
2. Сделать **ApplicationManager** классом **Singleton**: добавить статичный метод `getInstance()` и изменить модификатор
   доступа у конструктора **ApplicationManager** на `private`.
3. В методе `setUp()` у **TestBase** заменить инициализацию **ApplicationManager** на вызов статичного метода 
   `getInstance()`.
4. Создать **деструктор** для класса **ApplicationManager**.
5. Убрать метод `tearDown()` в классе **TestBase**, если он напрямую не активирует деструктор.
6. Проверить работоспособность тестов.
7. Создайте **ещё один тест** *(редактирование/удаление)*. Убедиться в правильной работе всех тестов.
8. Добавить в конец каждого теста **Assert'ы**, которые будут проверять правильность результата исполнения тестов.
9. Проверить работоспособность тестов.
